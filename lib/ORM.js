...
var _defaults = {
    ...
    integer: function (len) {
        return 'number('+(len||11)+')';
    },
    string: function (len) {
        return 'varchar2('+(len||255)+')';
    },
    date: function () {
        return 'date';
    }
...
Oracle_ORM.prototype.define = function(table_name, columns, options){
    ...
    if(!table_name) return ecode.para_error('Table name "'+table_name+'" is wrong.');
    ...
    for(var column in columns){
        ...
        if(typeof(col) == 'string'){
            if(!_defaults.hasOwnProperty(col)) return ecode.para_error('Column type"'+col+'" is not supported.');
            column_type_name = _defaults[col]();
        }else if(col.type){
            if(!_defaults.hasOwnProperty(col.type)) return ecode.para_error('Column type"'+col.type+'" is not supported.');
            column_type_name = _defaults[col.type](col.length);
        }else{
            continue;
        }
        ...
        define_obj.columns_obj[column_name] = column_type_name;
    }
    //sequence column
    ...
    if(options && options.indexes){
        for(var i= 0,l= options.indexes.length;i<l;i++){
            ...
            define_obj.table_indexes.push({
                sql : sql,
                fields : _obj.fields,
                unique: (_obj.type&&_obj.type.toLowerCase() == 'unique')
            });
        }
    }
...
Oracle_ORM.prototype.execute = function(){
    var pro = Q.defer(), self = this;
    ...
    if(_len == 0) {
        ...
        bindParams = arguments[1];
    } else if(_len == 3) {
        bindParams = arguments[1];
        options = extend(self.opt, arguments[2]);
    }
    sql = arguments[0]+'';
    self.getCon().then(function(con){
        var _pro = Q.defer();
        con.execute(sql,bindParams,options, function(e,r){
            if(e) _pro.reject({c:con,e:ecode.sql_execute('Sql execute: '+e.message)});
            else _pro.resolve({c:con, r:r});
        });
        return _pro.promise;
    }).then(function(res){
        res.c.release();
