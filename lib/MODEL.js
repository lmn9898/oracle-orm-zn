...
Model.prototype.max = function(){
    var pro = Q.defer(), self = this;
    var field = arguments[0], options = arguments[1] || {};
    if(!field){
        pro.reject(ecode.lack_para());
        return pro.promise;
    }
    ...
    options.model = self;
    makeCTW(options);
    var sql = 'select max('+field+') as max from '+_tab.substring(1,_tab.length)+(_where==''?'':(' where '+_where.substring(0,_where.length-4)));
    self.orm.getCon().then(function(con){
        var _pro = Q.defer();
        con.execute(sql,{},self.orm.opt,function(e,r){
            if(e) _pro.reject({c:con,e:'Max error: '+ e.message});
            else _pro.resolve({c:con,r: r.rows[0].MAX});
        });
        return _pro.promise;
    }).then(function(res){
        res.c.release();
...
Model.prototype.min = function(){
    ...
    options.model = self;
    makeCTW(options);
    var sql = 'select min('+field+') as min from '+_tab.substring(1,_tab.length)+(_where==''?'':(' where '+_where.substring(0,_where.length-4)));
    self.orm.getCon().then(function(con){
        var _pro = Q.defer();
        con.execute(sql,{},self.orm.opt,function(e,r){
            if(e) _pro.reject({c:con,e:'Min error: '+ e.message});
            else _pro.resolve({c:con,r: r.rows[0].MIN});
        });
        return _pro.promise;
    }).then(function(res){
        res.c.release();
...
Model.prototype.sum = function(){
    ...
    options.model = self;
    makeCTW(options);
    var sql = 'select sum('+field+') as sum from '+_tab.substring(1,_tab.length)+
        (_where==''?'':(' where '+_where.substring(0,_where.length-4)));
    self.orm.getCon().then(function(con){
        var _pro = Q.defer();
        con.execute(sql,{},self.orm.opt,function(e,r){
            if(e) _pro.reject({c:con,e:'Sum error: '+ e.message});
            else _pro.resolve({c:con,r: r.rows[0].SUM});
        });
        return _pro.promise;
    }).then(function(res){
        res.c.release();
...
Model.prototype.drop = function(){
    var pro = Q.defer(), self = this;
    var sql = 'drop table '+ self.table_name;
    self.orm.getCon().then(function (con) {
        var _pro = Q.defer();
        con.execute(sql,{},self.orm.opt,function (e) {
            if(e) _pro.reject({c:con,e:'Drop error: '+ e.message});
            else _pro.resolve({c:con,r: 0});
        });
        return _pro.promise;
    }).then(function (res) {
        res.c.release();
